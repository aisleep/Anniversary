...
- (void)setProgressFromNotification:(NSNotification *)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        id <MWPhoto> photoWithProgress = [dict objectForKey:MWPHOTO_NOTIFICATION_INFOKEY_PHOTO];
        if (photoWithProgress == _photo) {
//            NSLog(@"%f", [[dict valueForKey:@"progress"] floatValue]);
            float progress = [[dict valueForKey:MWPHOTO_NOTIFICATION_INFOKEY_PROGRESS] floatValue];
